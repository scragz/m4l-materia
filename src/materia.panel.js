...
function paint(){
 rect(0,0,1360,169,'live_surface_bg');rect(0,0,280,169,'live_lcd_bg');rect(1180,0,180,169,'live_lcd_bg');for(var i=0;i<22;i++){var col=Math.floor(i/11),row=i%11,x=8+col*137,y=9+row*14;text(names[i],x,y+8,9,'live_lcd_title');for(var b=0;b<8;b++)rect(x+43+b*9,y,7,9,(Math.floor((data[i*2]||0)/Math.pow(2,7-b))%2)?'live_value_arc':'live_lcd_frame');rect(x+119,y,7,9,data[i*2+1]?'live_value_arc':'live_lcd_frame');}
 tabs.forEach(function(t,i){var x=290+(i%10)*88,y=5+Math.floor(i/10)*22;rect(x,y,84,18,t===current?'live_selection':'live_control_bg');text(t,x+6,y+12,9);});
 ...
 text(data[44]?'RACE':'SETTLED',1190,131,9,data[44]?'live_value_bar_two':'live_control_fg');text(delayed+' delayed · 1×',1260,131,8);text(data[45]?'BER clock saturated':'',1190,145,8,data[45]?'live_value_bar_two':'live_control_fg');text(msg.substring(0,84),294,169-1,8);
